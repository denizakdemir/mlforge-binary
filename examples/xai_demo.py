...
The demo uses the enhanced XAI API:
- BinaryClassifier.explain() - For global model explanations with SHAP
- BinaryClassifier.explain_instance() - For local explanations of individual predictions

Key features:
1. Maps preprocessed features back to original business variables
2. Shows how categorical variables are represented after one-hot encoding
3. Aggregates importance metrics for better interpretability
4. Creates waterfall plots with actual feature values
5. Analyzes categorical variable impact on predictions
6. Includes fallback mechanisms for backward compatibility

This example shows how to create more interpretable and business-friendly
explanations from ML models by maintaining the connection to original features.
"""
...
def extract_original_feature_importance(clf, X_test, feature_mapping, categorical_cols, continuous_cols):
    """Extract feature importance and map back to original features."""
    print("\n=== Extracting Original Feature Importance ===")
    
    # Get processed feature importance - try the new XAI API first
    processed_importance = {}
    try:
        # Try the new explain() API with test data for SHAP
        print("Attempting to use the new XAI API...")
        ...
        if 'feature_importance' in explanations and explanations['feature_importance']:
            ...
            print(f"Successfully extracted {len(processed_importance)} feature importances using the new API")
            ...
            print("No feature importance found in explain() result, trying fallback...")
            
    except Exception as e:
        print(f"New API explain() method failed: {e}")
        print("Falling back to legacy methods...")
        
        # Legacy methods for extracting feature importance
        try:
...
def create_original_waterfall_plot(clf, X_train, X_test, y_train, y_test, feature_mapping, categorical_cols, continuous_cols, dataset_name, instance_idx=0):
    ...
    try:
        ...
        # First try the new XAI API for instance-level explanation
        print("Attempting to use the new XAI API for waterfall plot...")
        try:
            # Get instance explanation using the new API
            explanation = clf.explain_instance(instance_df)
            
            if explanation and 'shap_values' in explanation:
                print("Successfully retrieved instance explanation using the new API")
                
                # Check if we already have a waterfall plot
                if 'waterfall_plot' in explanation and explanation['waterfall_plot'] is not None:
                    print("Using built-in waterfall plot")
                    
                    # Display the waterfall plot if available
                    plt.figure(figsize=(16, 10))
                    plt.imshow(explanation['waterfall_plot'])
                    plt.axis('off')
                    ...
                    # Extract shap values for manual mapping
                    shap_values = explanation['shap_values']
                    processed_feature_names = explanation.get('feature_names', [])
                    
                    # If we don't have feature names, try to get them from the classifier
                    if not processed_feature_names:
                        try:
                            processed_feature_names = clf.get_feature_names_out()
                        except:
                            shap_values_to_use = shap_values[0] if isinstance(shap_values, np.ndarray) and len(shap_values.shape) > 1 else shap_values
                            processed_feature_names = [f'feature_{i}' for i in range(len(shap_values_to_use))]
                
                # If we have shap values but no waterfall plot, continue with manual plotting
                shap_values_to_use = None
                if isinstance(shap_values, np.ndarray):
                    # Handle different SHAP value formats
                    if len(shap_values.shape) == 3:
                        shap_values_to_use = shap_values[0, :, 1]  # Use positive class for first instance
                    elif len(shap_values.shape) == 2:
                        shap_values_to_use = shap_values[0, :]  # First instance
                    elif len(shap_values.shape) == 1:
                        shap_values_to_use = shap_values  # Already for single instance
                    else:
                        print(f"Unexpected SHAP values shape: {shap_values.shape}")
                        raise ValueError("Unexpected SHAP values shape")
                else:
                    # Try to extract values from SHAP Explanation object
                    if hasattr(shap_values, 'values'):
                        shap_array = shap_values.values
                        if len(shap_array.shape) == 3:
                            shap_values_to_use = shap_array[0, :, 1]
                        elif len(shap_array.shape) == 2:
                            shap_values_to_use = shap_array[0, :]
                        else:
                            print(f"Unexpected SHAP values shape: {shap_array.shape}")
                            raise ValueError("Unexpected SHAP values shape")
                
                if shap_values_to_use is not None:
                    print(f"Using SHAP values with shape: {shap_values_to_use.shape}")
                    ...
                    raise ValueError("No SHAP values extracted")
                
            else:
                print("No SHAP values found in explain_instance() result, falling back to manual implementation")
                raise ValueError("No SHAP values in explanation")
                
        except Exception as e:
            print(f"New API waterfall plot attempt failed, falling back to manual implementation: {e}")
            
            # Legacy implementation
            print("Using legacy SHAP implementation...")
...
def demonstrate_original_shap_analysis(clf, X_train, X_test, feature_mapping, categorical_cols, continuous_cols, dataset_name):
    ...
    try:
        ...
        # First try to use the new XAI API
        print("Attempting to use the new XAI API for SHAP analysis...")
        try:
            # Get SHAP explanations using the new API
            explanations = clf.explain(X_test[:30])
            
            if 'shap_values' in explanations and explanations['shap_values'] is not None:
                print("Successfully retrieved SHAP values using the new API")
                
                # Get the shap values from the explanations
                if hasattr(explanations['shap_values'], 'values'):
                    # For newer SHAP versions that return Explanation objects
                    shap_array = explanations['shap_values'].values
                else:
                    # For older versions or if already numpy array
                    shap_array = explanations['shap_values']
                
                # Get the feature names
                processed_feature_names = explanations.get('feature_names', [])
                if not processed_feature_names and 'feature_importance' in explanations:
                    processed_feature_names = list(explanations['feature_importance'].keys())
                
                # If we still don't have feature names, try to get them from the classifier
                if not processed_feature_names:
                    try:
                        processed_feature_names = clf.get_feature_names_out()
                    except:
                        # Create generic feature names if we can't get real ones
                        n_features = shap_array.shape[1] if len(shap_array.shape) >= 2 else 0
                        processed_feature_names = [f'feature_{i}' for i in range(n_features)]
                
                print(f"Got {len(processed_feature_names)} feature names")
                
            else:
                print("No SHAP values found in explain() result, falling back to manual implementation")
                raise ValueError("No SHAP values in explanations")
                
        except Exception as e:
            print(f"New API SHAP analysis attempt failed, falling back to manual implementation: {e}")
        ...
        # Use only the test samples that we used for SHAP analysis
...
def main():
    ...
    # Extract original feature importance using the new API with test data
    cat_importance, cont_importance, cat_detailed = extract_original_feature_importance(
        clf, X_test, feature_mapping, categorical_cols, continuous_cols
    )
    ...
    print("=" * 60)
    print("\nKey Insights:")
    print("1. The new XAI API provides consistent explanations across different model types")
    print("2. Feature importance is aggregated back to original categorical variables")
    print("3. Category-level importance shows which specific categories matter most")
    print("4. SHAP values are properly mapped to original features, not dummy variables")
    print("5. Categorical variables show their total impact across all encoded features")
    print("6. Waterfall plots explain individual predictions using original feature names")
    print("7. Categorical variables show specific category values and their impact")
    print("8. Analysis provides actionable insights about original business variables")
    ...
    print("\nNew XAI API advantages:")
    print("- Simplified workflow for explaining model predictions")
    print("- Consistent interface for different model types")
    print("- Built-in visualizations for explanations")
    print("- Original feature mapping for better business interpretability")
